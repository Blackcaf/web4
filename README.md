...
#### `Web4Application.java`
Точка входа Spring Boot приложения. Запускает встроенный сервер и инициализирует контекст.

#### `JerseyConfig.java`
Конфигурация JAX-RS (Jersey). Регистрирует все Resource классы для обработки HTTP запросов.

#### `SecurityConfig.java`
Конфигурация Spring Security: CORS, CSRF, CSP, session management.

#### `AuthResource.java`
JAX-RS Resource для эндпоинтов аутентификации (`/api/auth/*`). Обрабатывает регистрацию, вход, OAuth.

#### `ResultResource.java`
JAX-RS Resource для работы с результатами (`/api/results/*`). Проверка точек, получение истории.

#### `AuthService.java`
Бизнес-логика аутентификации: регистрация пользователей, проверка паролей, OAuth интеграция.

#### `ResultService.java`
Бизнес-логика проверки точек: вызов `AreaCheckUtil`, сохранение в БД, получение истории.

#### `AreaCheckUtil.java`
Математическая логика проверки попадания точки в область (прямоугольник, треугольник, окружность).

#### `JwtUtil.java`
Генерация и валидация JWT токенов. Использует библиотеку `io.jsonwebtoken`.

#### `PasswordHashingService.java`
Хеширование и проверка паролей с помощью BCrypt (Spring Security Crypto).

#### `LoginAttemptService.java`
In-memory отслеживание неудачных попыток входа. Определяет необходимость капчи.

#### `RateLimitService.java`
In-memory rate limiting по IP адресу. Защита от DDoS и спама.

#### `CaptchaService.java`
Взаимодействие с Google reCAPTCHA API для верификации пользователя.

#### `AuditService.java`
Логирование событий безопасности: вход, регистрация, подозрительная активность.

#### `OAuthProvider.java` / `GoogleOAuthProvider.java` / `YandexOAuthProvider.java`
Strategy Pattern для OAuth 2.0 провайдеров. Обмен authorization code на access token и получение email.

#### `TokenExtractor.java`
Извлечение и валидация JWT токена из HTTP заголовка `Authorization`.

#### `HttpRequestHelper.java`
Извлечение IP адреса клиента (учитывая прокси) и User-Agent из HTTP запроса.

#### `ResponseBuilder.java`
Утилита для создания стандартизированных JSON ответов об ошибках.

#### `ValidationExceptionMapper.java`
JAX-RS ExceptionMapper для обработки ошибок Jakarta Validation и возврата понятных сообщений клиенту.
...
#### `app.routes.ts`
Конфигурация маршрутизации Angular: `/login`, `/main`, `/403`. Использует `AuthGuard`.

#### `app.config.ts`
Глобальная конфигурация Angular: провайдеры, HTTP интерцепторы, маршрутизация.

#### `auth.guard.ts`
Route Guard для защиты маршрута `/main`. Проверяет наличие JWT токена.

#### `auth.interceptor.ts`
HTTP Interceptor для автоматического добавления JWT токена в заголовок `Authorization: Bearer <token>`.

#### `auth.service.ts`
Сервис для взаимодействия с API аутентификации. Методы: `register()`, `login()`, `socialLogin()`, `logout()`.

#### `result.service.ts`
Сервис для взаимодействия с API результатов. Методы: `checkPoint()`, `getUserResults()`, `clearResults()`.

#### `theme.service.ts`
Управление темной/светлой темой. Сохранение предпочтений в cookies (срок хранения 365 дней).

#### `login.component.ts`
Компонент страницы входа/регистрации. Логика форм, анимация монстра, reCAPTCHA, OAuth кнопки.

#### `main.component.ts`
Компонент основной страницы. Рисование графика на Canvas, обработка кликов, отправка запросов, таблица результатов.

#### `monster-animation.manager.ts`
Класс для управления анимацией монстра на странице логина (состояния, моргание, реакция на мышь).

#### `captcha.manager.ts`
Класс для управления Google reCAPTCHA (отображение, скрытие, получение токена, сброс).

#### `password-validator.ts`
Класс для визуальной индикации длины пароля (не валидация, только UI feedback).

#### `cookie.service.ts`
Утилита для работы с HTTP Cookies. Методы: `set()`, `get()`, `delete()`, `exists()`. Используется для хранения JWT токена, username и темы с флагами `SameSite=Strict` для защиты от CSRF атак.
...
### Страница Login
- **Анимированный монстр** - реагирует на фокус полей, движение мыши
- **Валидация в реальном времени** - отображение ошибок
- **Индикатор силы пароля** - визуальная шкала
- **Google reCAPTCHA** - появляется после неудачных попыток
- **OAuth кнопки** - вход через Google/Yandex
- **Переключатель темы** - светлая/темная тема

### Страница Main
- **3 секции с прокруткой:**
  1. Инфо карточка (студент, группа, вариант)
  2. График + форма ввода
  3. Таблица результатов
- **Интерактивный Canvas график:**
  - Рисование области
  - Клик для добавления точки
  - Hover подсветка координат
  - Автоматическая отрисовка всех точек
- **Таблица результатов:**
  - Анимированные строки
  - Бейджи "Попадание"/"Промах"
  - Время и скорость выполнения
  - Вертикальный скролл
...
- Pulse
- Shimmer
- Все transition с cubic-bezier для плавности
